nums = '';
store = [];
i = 0;
total = [];
while i<len(nums):
    store.append(int(nums[i:i+2]));
    
    if len(store) == 20:
        total.append(list(store));
        store.clear();
        print('h');
    i+=2;
print(total);
big = 0;
r = 0;
c = 0;
while r< len(total):
    c = 0;
    while c<20:
        if c-4>=0:
            if total[r][c]*total[r][c-1]*total[r][c-2]*total[r][c-3]>big:
                big = total[r][c]*total[r][c-1]*total[r][c-2]*total[r][c-3];
            if r-4>=0:
                if total[r][c]*total[r-1][c-1]*total[r-2][c-2]*total[r-3][c-3] > big:
                    big = total[r][c]*total[r-1][c-1]*total[r-2][c-2]*total[r-3][c-3];
            if r+4<=20:
                if total[r][c]*total[r+1][c-1]*total[r+2][c-2]*total[r+3][c-3] > big:
                    big = total[r][c]*total[r+1][c-1]*total[r+2][c-2]*total[r+3][c-3];
        if c+4<=20:
            if total[r][c]*total[r][c+1]*total[r][c+2]*total[r][c+3]>big:
                big = total[r][c]*total[r][c+1]*total[r][c+2]*total[r][c+3];
            if r-4>=0:
                if total[r][c]*total[r-1][c+1]*total[r-2][c+2]*total[r-3][c+3] > big:
                    big = total[r][c]*total[r-1][c+1]*total[r-2][c+2]*total[r-3][c+3];
            if r+4<=20:
                if total[r][c]*total[r+1][c+1]*total[r+2][c+2]*total[r+3][c+3] > big:
                    big = total[r][c]*total[r+1][c+1]*total[r+2][c+2]*total[r+3][c+3];
        c+=1;
    r+=1;
print(big);
